    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <form onSubmit={handleSubmit} className="relative glass-panel w-full max-w-md p-6 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold">{isEdit ? t("vault.portalModal.updateTitle") : t("vault.portalModal.createTitle")}</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        {isEdit ? (
          <p className="text-xs text-slate-500 dark:text-slate-400 -mt-1">
            {t("vault.portalModal.editNotice")}
          </p>
        ) : (
          <p className="text-xs text-slate-500 dark:text-slate-400 -mt-1">
            {t("vault.portalModal.createNotice")}
          </p>
        )}
